	...
	public void to_findRepeatedElements() {
		...
		for (int i = 0; i < arr1.length; i++) // i=0 0 0 1 2 2 ... 3 3 ....
		{
			for (int j = 0; j < arr2.length; j++) // j=0 1 2 0 0 1 ... 0 1 .....
			{
				...
				{
					count++; // to find how many repeated elements so that we can assign the length of new
								// array. count=1 count=2 count=3;
					break;
				}
			}
		}
		System.out.println("\nRepeated elements count: " + count);
	}
	...
	public void printArray_OfCommonElements() {
		...
		for (int i = 0; i < arr1.length; i++) {
			for (j = 0; j < arr2.length; j++) {
				if (arr1[i] == arr2[j]) // it a number repeats then enter into this block.
				{
					b[index] = arr1[i]; // assigning that index value to new array.
					index++; // then index increases to 1....
